 ...
 *
 * C1 freezes the set; FR-002 fixes the values; SC-009 forbids a twelfth. `SlowLockPaletteTest`
 * asserts the count, each literal and the contrast of every pairing in [TextPairings], so a colour
 * added without a pairing, or a literal nudged by a digit, fails the build rather than a review.
 *
 * These are the *source*: the Material 3 `ColorScheme` in `Theme.kt` is derived from them, never
 * the reverse, and no screen may declare a colour of its own — the test scans the source tree for
 * that.
 */
...
/**
 * The accent. A fill, a border, and a rule — never a glyph on the ground: amber on [Bone] measures
 * 2.76:1, under the 4.5:1 floor (C2). It is legible only the other way round ([Ink] on amber is
 * 5.82:1, which the primary action uses). For an accent-coloured *word*, the token is [AmberDark].
 */
 ...
 *
 * Two deliberate divergences from the design source (C1, C9): the artboard draws the inactive
 * slider track at `#E4DFD4` — three points per channel from this token and indistinguishable — and
 * the row divider at this value rather than at [Line].
 */
val Fill = Color(0xFFE7E2D7)

/** The declared set, by name, so the count is something a test asserts rather than a reviewer. */
...
/**
 * Every text-on-surface pairing the design uses.
 *
 * C3 makes this binding in both directions: each pairing must clear 4.5:1, and a pairing not in
 * this list is not permitted on screen. `Amber to Bone` is deliberately absent — it fails at
 * 2.76:1, which is the whole reason [AmberDark] exists.
 */
