...
/**
 * The matrices are invisible until they are permanent: a wrong one produces an icon that looks
 * plausible in the preview, gets baked into a pinned shortcut, and stays on the user's home
 * screen — where SlowLock can neither find it nor fix it. Manual testing catches a *blatantly*
 * wrong icon; it does not catch a subtly wrong one, and it cannot run before the pin.
 *
 * These assertions are also what makes the "literal constants, no framework call" rule in
 * [IconTreatment] enforceable. Under `isReturnDefaultValues = true` a derived `Gray` would come
 * back empty and this class would pass while asserting nothing.
 */
class IconTreatmentTest {
    ...
    /**
     * The alpha-row trap (research.md R7, manual case M1.5). An inverted alpha row turns
     * transparent icon corners opaque black and makes every inverted icon a solid square.
     */
    @Test
