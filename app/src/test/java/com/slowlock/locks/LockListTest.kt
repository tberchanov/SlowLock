 ...
 *
 * Every failure these assertions catch is **silent**. A renamed file or key compiles clean, reads
 * as absent, and empties every user's Locks screen after an update they did not ask for — while
 * their home-screen icons keep working, so the app looks fine and the locks are simply gone
 * (`contracts/lock-store.md`, L1). There is no build-time check and no migration; this class is
 * the check.
 *
 * These are pure functions by design: [LockStore] holds only the wiring, because the JVM suite
 * runs with `isReturnDefaultValues = true` and a test written against `SharedPreferences` would
...
class LockListTest {
    ...
    /**
     * FR-013: a duplicate collapses to its **first** position. Later is not newer — a row that
     * moved would look to the user like the lock had been re-made.
     */
    @Test
    ...
    /**
     * FR-007, L4: **nothing on the read path throws**, and whatever comes back is already clean.
     *
     * A malformed value is somebody's real device on a cold start. The honest answer is a list —
     * possibly empty — never an exception, and never an entry that is blank or untrimmed.
     */
    @Test
