 ...
 * (FR-003a).
 *
 * Every row of [deriveLocks]'s table is asserted here, including the one that matters most and is
 * invisible on a happy path: a declined pin dialog creating nothing.
 */
class LockDeriveTest {
    ...
    fun `declining the pin creates nothing`() {
        // The case this design exists for. Nothing was pinned, so there is nothing to show — and
        // no record was written at the tap that would have to be cleaned up.
    ...
    @Test
    fun `declining leaves the locks that already existed untouched`() {
    ...
    fun `a shortcut pinned without the app being told still becomes a lock`() {
        ...
        // callback, so this case needs no handling of its own.
        val result = deriveLocks(
            cached = listOf("com.a"),
    ...
    fun `there is no way for the app to hide a pinned lock`() {
        // The whole of FR-021 as it now stands: removal happens on the home screen, so a pinned
        // shortcut always yields a row. This test exists to fail loudly if an in-app "remove"
        // that merely hides the row is ever reintroduced — the icon would stay, still waiting and
        // still opening the app, and the list would stop meaning what it says.
