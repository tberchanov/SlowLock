...
- [ ] T044 Run manual section M5 in `manual-test-plan.md` on a **physical device**: `adb shell am force-stop com.slowlock` then tap the shortcut, and a real `adb reboot` then tap it again without opening SlowLock first (FR-017, SC-007). A failure here means the launch path depends on SlowLock's process state, which the design forbids (depends on T042)
- [ ] T045 Run manual section M7 in `manual-test-plan.md` to confirm feature 001 has not regressed — enumeration, ordering, search, and the empty/no-results states all behave as before (depends on T031)
